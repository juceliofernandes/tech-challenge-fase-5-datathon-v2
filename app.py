...
def carregar_artefato():
    if ARTEFATO.exists():
        ...
        return pacote, None
    # fallback: treino rápido se planilha existir
    loc = localizar_planilha()
    if loc is None or not loc.exists():
        return None, "Artefato não encontrado e planilha ausente. Rode o notebook 02 primeiro ou envie outputs/modelo/modelo_risco_defasagem.joblib."
    try:
        ...
        partes = [padronizar_aba(pd.read_excel(loc, sheet_name=a), a) for a in ["PEDE2022", "PEDE2023", "PEDE2024"]]
        ...
        COLS = NUM + CAT
        X, y = pares[COLS], pares["risco_proximo_ano"]
        ...
        pipe = ImbPipeline(
            [("prep", prep), ("bal", RandomOverSampler(random_state=42)), ("modelo", RandomForestClassifier(n_estimators=300, random_state=42, n_jobs=-1, class_weight="balanced"))]
        )
        pipe.fit(X, y)
        # limiar simples 0.5 no fallback
        pacote = {"pipeline": pipe, "threshold": 0.5, "feature_columns": COLS, "model_name": "RandomForest (treino emergencial no app)", "test_metrics": {}, "global_importances": []}
        return pacote, "Artefato não estava no disco — treinei um modelo emergencial com a planilha local. Para métricas auditadas, rode o notebook 02 e faça o commit do .joblib."
    except Exception as exc:  # pragma: no cover
        return None, f"Falha no treino emergencial: {exc}"
...
3. Python 3.11. Não é necessário `secrets.toml`.
