...
def carregar_artefato():
    ...
    if ARTEFATO.exists():
        try:
            ...
            aviso = f"Modelo salvo incompatível com esta versão de scikit-learn ({exc}). Treinando um novo na inicialização (sem impacto no uso)."
...
with tab_ind:
    ...
    # Importâncias globais — visíveis sem precisar clicar em "Calcular risco"
    imp = PACOTE.get("global_importances") or []
    if imp:
        st.divider()
        st.subheader("Importância global das variáveis (modelo em produção)")
        df_imp = pd.DataFrame(imp)
        st.bar_chart(df_imp.set_index("variavel")["importancia_global"])
    else:
        st.caption("Importâncias globais indisponíveis para este modelo.")
